...
……ごめんなさい、その通りです。example08までの方式だと、光源の位置をカメラ以外に変更したなら、影になるはずの部分が影にならなくて変な画像になってしまうでしょう。そこで、example09で影を生成するようにプログラムを修正します。example09ではとりあえず、光が直接当たる場合は明るく、当たらない場合は暗くするという簡単な方式を実現してみます。
