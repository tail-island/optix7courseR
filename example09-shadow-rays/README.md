...
すみません、その通りです。example08までの方式だと、光源の位置をカメラ以外に変更した場合に、光が届かずに影になるはずの部分が影にならなくて変な画像になってしまうんです。example09では、影を生成するようにプログラムを修正していきましょう。example09では、光が直接当たる場合は明るく、当たらない場合は暗くするという簡単な方式で考えてみます。
